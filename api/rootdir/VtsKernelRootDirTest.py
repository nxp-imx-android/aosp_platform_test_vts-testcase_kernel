...
_INIT_RC_DIR = os.path.join("vts", "testcases", "kernel", "api",
                            "rootdir", "init_rc_files")

# The allowed *.rc files, where file content will be checked.
_ALLOWED_RC_FILES = set("/" + os.path.basename(rc_file) for rc_file in
                        glob.glob(os.path.join(_INIT_RC_DIR, "*.rc")))
...
# init.environ.rc.in, which is not included in _INIT_RC_DIR.
        ...
        def _CheckFiles(dir_path):
            ...
            for f in current_files - unexpected_files:
                if f in _ALLOWED_RC_FILES:
                    ...
                    if not filecmp.cmp(os.path.join(_INIT_RC_DIR, f[1:]),
                                       os.path.join(self._temp_dir, f[1:])):
                        error_msg.append(
                            "Unexpected file content: %s, please check: %s" % (
                                f, os.path.join(_INIT_RC_DIR, f[1:])))
